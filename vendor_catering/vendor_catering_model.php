    <?php 
        ...
        $link = getLinkDatabase();
            ...
            $name = secure_input($_POST['name']);
                ...
                $availabilityErr = availabilityValidation($availability);
        ...
        function editVendorCatering($id_vendor_catering){
            ...
            if (isset($_POST['updateDataSubmit'])) {
                ...
                $availabilityErr = availabilityValidation($_POST['availability']);
                ...
                if ( ($nameErr=="") && ($phoneNumberErr=="") && ($addressErr=="") && ($availabilityErr=="") ) {
                    ...
                    $res = mysqli_query($link,$sql);
                        if ($res) {
                            header('Location: vendor_catering.php');
                        } else {
        ...
        function saveDataVendorCatering($link,$data){
            ...
            if ($res) {
                ...
                header('Location: vendor_catering_add.php');
            }
        }
        ...
        function availabilityValidation($availability){
            $availabilityErr="";
                if (empty($availability)) {
                    $availabilityErr = "Availability is required";
                } elseif (!is_numeric($availability)) {
                    $availabilityErr = "Availability invalid input format";
                } elseif (($availability == 0) and ($availability==1)) {
                    $availabilityErr = "Availability not must be on or off";
                } else {
                    $availabilityErr = "";
                }
